...
const StockHistory = () => {
    ...
    useEffect(() => {
        const fetchHistorique = async () => {
            try {
                const response = await axios.get('http://172.31.249.204:8089/stock/historique');
                setHistorique(response.data); // Mettre à jour l'état avec les données de l'historique
            } catch (error) {
