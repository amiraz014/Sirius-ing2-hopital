...
const StockHistoryByMedicament = () => {
    ...
    useEffect(() => {
        const fetchHistoriqueParMedicament = async () => {
            try {
                const response = await axios.get('http://172.31.249.204:8089/stock/historique/par-medicament');
                setMedicaments(response.data); // Mettre à jour l'état avec les données
            } catch (error) {
