    ...
    useEffect(() => {
        const fetchMedicamentDetails = async () => {
            try {
                ...
                const responseMedicament = await axios.get(`http://172.31.249.204:8089/stock/historique/par-medicament`);
                const medicamentData = responseMedicament.data.find((m) => m.idMedicament === parseInt(id));
                setMedicament(medicamentData);

                const responseSorties = await axios.get(`http://172.31.249.204:8089/stock/historique`);
